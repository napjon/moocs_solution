...
def link_spam1(graph,k):#Do Not remove completely, it will change the length of each element in dictionary
    if k == 0:
        for a in graph:
            if a in graph[a]:
                graph[a].remove(a)
        ...
        for a in graph:#search every element a in graph
            for b in graph:#that each element b in element graph a
...
def link_spam(graph,k):#Do Not remove completely, it will change the length of each element in dictionary
    for a in graph:#search every element a in graph
        if k == 0:
            if a in graph[a]:
                graph[a].remove(a)
        ...
        for b in graph:#that each element b in element graph a
            ...
            for c in graph:
                if k >= 2:
                    #a in graph c, b in graph a, c in graph b
                    if a in graph[c] and b in graph[a] and c in graph[b]:
                        graph[a].remove(b)
                        graph[a].append('#')
                        graph[b].remove(c)
                        graph[b].append('#')
...
def compute_ranks(graph, k):
    ...
    numloops = 10
    ranks = {}
    graph = link_spam(graph, k)
    npages = len(graph)
    for page in graph:
        ranks[page] = 1.0 / npages#initial belief, so pages all have the same chance
    for i in range(0, numloops):
        newranks = {}#create a temporary dictionary, to store a newrank calculation
        for page in graph:
            newrank = (1 - d) / npages#new rank always initialize for 1-d
            for node in graph:
                if page in graph[node]:
                    newrank = newrank + d * (ranks[node]/len(graph[node]))#every loops has include a different chance
